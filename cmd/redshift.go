...
func NewRedshiftCmd() *cobra.Command {
	...
	run := func() error {
		...
		if sourceOpts.format == SourceFormatIceberg {
			...
			for _, tableFQN := range tables {
				...
				rowApplier, err := redshiftsql.NewRedshiftRowApplier(
					&redshiftConfigFromCli,
					fmt.Sprintf("iceberg_temp_%s_%s", sourceDatabase, sourceTable),
					storageURI,
					credValue,
				)
				if err != nil {
		...
		for _, tableFQN := range tables {
			...
			snapConnector, err := redshiftsql.NewRedshiftConnector(
				&redshiftConfigFromCli,
				fmt.Sprintf("snapshot_temp_%s_%s", sourceDatabase, sourceTable), // Actually, this is not used
				snapshotURI,
				credValue,
			)
			if err != nil {
			...
			increConnector, err := redshiftsql.NewRedshiftConnector(
				&redshiftConfigFromCli,
				fmt.Sprintf("increment_temp_%s_%s", sourceDatabase, sourceTable),
				incrementURI,
				credValue,
			)
			if err != nil {
	...
	cmd.Flags().StringVar(&redshiftConfigFromCli.Database, "redshift.database", "dev", "redshift database")
	cmd.Flags().StringVar(&redshiftConfigFromCli.Schema, "redshift.schema", "public", "redshift schema")
